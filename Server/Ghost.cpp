...
void CGhost::OnTraceMove(float elapsedTime)
{
	...
	if (!targetPlayer || targetPlayer->GetIsPossessed() || targetPlayer->GetReturned() || targetPlayer->IsIncapacitated()) {
		target_player.reset();
...
void CGhost::OnAttackMove(float elapsedTime)
{
	...
	// 돌진: STOP_DIST 이상이면 달리고, 코앞에 닿으면 즉시 멈춤
